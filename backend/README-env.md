...
4. `OPENAI_TIMEOUT_SECONDS` (default `20`)
5. `OPENAI_MAX_RETRIES` (default `2`)
6. `OPENAI_TEMPERATURE_ORACLE` (default `0.7`)
7. `OPENAI_TOP_P_ORACLE` (default `1.0`)
8. `OPENAI_MAX_TOKENS_ORACLE` (default `600`)
9. `OPENAI_TEMPERATURE_CV` (default `0.2`)
10. `OPENAI_TOP_P_CV` (default `1.0`)
11. `OPENAI_MAX_TOKENS_CV` (default `900`)
12. `FRONTEND_URL` (default `http://localhost:5173`)
13. `DB_PATH` (default `data`)
14. `LOG_LEVEL` (default `INFO`)
15. `LOG_REDACTION_ENABLED` (default `true`)

...
14. `OPENAI_ORACLE_TOOL_ROUND_LIMIT` (default `3`)
15. `LLM_MAX_INSTRUCTIONS_CHARS` (default `12000`)
16. `LLM_MAX_INPUT_CHARS` (default `24000`)
17. `LLM_MAX_TOTAL_CHARS` (default `32000`)
18. `OPENAI_TEMPERATURE_CV` (default `0.2`)
19. `OPENAI_TOP_P_CV` (default `1.0`)
20. `OPENAI_MAX_TOKENS_CV` (default `900`)
21. `OPENAI_TEMPERATURE_REPO` (default `0.2`)
22. `OPENAI_TOP_P_REPO` (default `1.0`)
23. `OPENAI_MAX_TOKENS_REPO` (default `900`)
24. `FRONTEND_URL` (default `http://localhost:5173`)
25. `DB_PATH` (default `data`)
26. `LOG_LEVEL` (default `INFO`)
27. `LOG_REDACTION_ENABLED` (default `true`)
